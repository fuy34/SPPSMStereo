...
def main():
    ...
    for inx in range(len(test_left_img)):
        ...
        # if inx < 10:
        #     cv2.imwrite(args.savepath + '/{}.png'.format(name), (100 * pred_disp).astype(np.uint16))
        #     cv2.imwrite(args.savepath + '/{}_gt.png'.format(name), (100 * gt_disp).astype(np.uint16))
        #     cv2.imwrite(args.savepath + '/{}_err.png'.format(name), (val2uint8(err, 20)))
        #     cv2.imwrite(args.savepath + '/{}_mask.png'.format(name), mask + 1.)
        # # output
        if True:
            top_cut = 640
