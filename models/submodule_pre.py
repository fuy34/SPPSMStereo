...
def conv( in_planes, out_planes, kernel_size=3, stride=1, batchNorm =True):
    if batchNorm:
...
# def spPredMask(in_planes, out_planes, kernel_size=3, stride=1):
#     return nn.Sequential(
#         nn.Conv2d(in_planes, max(in_planes//4, 16), kernel_size=kernel_size, stride=stride, padding=(kernel_size - 1) // 2,
#                   bias=False),
#         nn.BatchNorm2d(max(in_planes//4, 16)),
#         nn.ReLU(inplace=True),
#         nn.Conv2d(max(in_planes//4, 16), out_planes, kernel_size=kernel_size, stride=stride, padding=(kernel_size - 1) // 2,
#               bias=True),
#         nn.Softmax(dim=1)
#     )
...
class feature_extraction_sp16(nn.Module):
    def __init__(self):
        super(feature_extraction_sp16, self).__init__()
        self.inplanes = 32
